% =========================================================================
% File: Step0_Motion_Event_Detection.m
...
cfg.outputDir = fullfile(scriptDir, 'EventDetectionResults');
...
function [events, rejected, scheduleOffset] = detectMotionEvents(t, envelope, threshold, lowThreshold, cfg)
    ...
    for rep = 1:numel(slotCenters)
        ...
        [ev, hasCandidate] = detectEventInSlot(t, envelope, threshold, ...
            lowThreshold, slotStart, slotEnd, rep, cfg);
        if hasCandidate
...
function [ev, hasCandidate] = detectEventInSlot(t, envelope, threshold, lowThreshold, slotStart, slotEnd, rep, cfg)
    slotIdx = find(t >= slotStart & t <= slotEnd);
    hasCandidate = false;
    ev = makeEvent(1, 1, t, envelope);
    if isempty(slotIdx)
        return;
    end
    ...
    startIdx = max(1, startIdx - pad);
    endIdx = min(numel(t), endIdx + pad);
    if endIdx <= startIdx
        return;
    end
    ...
    ev.repIndex = rep;
    ev.reason = 'scheduled_slot_candidate';
    hasCandidate = true;
end
