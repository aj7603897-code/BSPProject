% =========================================================================
% File: Step0_Motion_Event_Detection_Unscheduled_HighQuality.m
% Purpose: Detect, count, and segment high-quality complete EMG motion cycles.
%
% Experiment protocol:
...
%
% Outputs:
%   EventDetectionResults_Unscheduled_HighQuality/summary.csv
%   EventDetectionResults_Unscheduled_HighQuality/event_boundaries.csv
%   EventDetectionResults_Unscheduled_HighQuality/segmented_records/<record>/<record>_repXX.csv
%   EventDetectionResults_Unscheduled_HighQuality/figures/<record>_events.png
% =========================================================================
...
cfg.outputDir = fullfile(scriptDir, 'EventDetectionResults_Unscheduled_HighQuality');
...
cfg.enableQualityReject = true;
cfg.minRelativePeakFraction = 0.50;
cfg.minRelativeMeanFraction = 0.45;
cfg.qualityReferenceTopFraction = 0.50;
cfg.minEventsForQualityCheck = 3;
...
for i = 1:numel(csvFiles)
    ...
    try
        ...
        [events, rejected] = rejectLowQualityEvents(events, rejected, cfg);
...
function [validEvents, rejected] = rejectLowQualityEvents(events, rejected, cfg)
    validEvents = events;
    if ~cfg.enableQualityReject || numel(events) < cfg.minEventsForQualityCheck
        return;
    end

    peakValues = [events.peakValue];
    meanValues = [events.meanValue];
    referencePeak = robustTopReference(peakValues, cfg.qualityReferenceTopFraction);
    referenceMean = robustTopReference(meanValues, cfg.qualityReferenceTopFraction);
    if ~isfinite(referencePeak) || referencePeak <= 0 || ...
            ~isfinite(referenceMean) || referenceMean <= 0
        return;
    end

    keep = true(1, numel(events));
    for i = 1:numel(events)
        peakTooWeak = events(i).peakValue < cfg.minRelativePeakFraction * referencePeak;
        meanTooWeak = events(i).meanValue < cfg.minRelativeMeanFraction * referenceMean;
        if peakTooWeak || meanTooWeak
            ev = events(i);
            ev.reason = 'low_quality_relative_amplitude';
            rejected(end + 1) = ev; %#ok<AGROW>
            keep(i) = false;
        end
    end
...
function ref = robustTopReference(values, topFraction)
    values = values(:);
    ...
    nTop = max(1, ceil(numel(values) * topFraction));
    ref = median(values(1:nTop));
end
